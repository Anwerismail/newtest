    ...
    if (!this.ticketNumber && this.isNew) {
        try {
            ...
            const lastTicket = await mongoose.model('Ticket')
                .findOne({ ticketNumber: new RegExp(`^EVO-${year}-`) })
                .sort({ ticketNumber: -1 })
                .select('ticketNumber')
                .lean();
            ...
            this.ticketNumber = `EVO-${year}-${String(nextNumber).padStart(4, '0')}`;
        } catch (error) {
...
TicketSchema.methods.canModify = function(user) {
    ...
    const reporterId = this.reporter._id || this.reporter;
    if (reporterId.toString() === user._id.toString()) return true;

    // Assigné peut modifier
    if (this.assignedTo) {
        const assignedToId = this.assignedTo._id || this.assignedTo;
        if (assignedToId.toString() === user._id.toString()) return true;
    }
